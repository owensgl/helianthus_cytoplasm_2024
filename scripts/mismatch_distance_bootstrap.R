...
tree <- read.tree("../fasta/wild.snps.v2.cp.noindels.fa.treefile")


tree <-collapseUnsupportedEdges(tree, cutoff=50)
...
summarized_dist <- distance_percentile %>%
  inner_join(final_diagnostic) %>%
  dplyr::filter(species != "Deb", species != "Ano") %>%
  group_by(species) %>%
...
pdf("figures/mismatch_distance.bootstrap.v2.pdf",height=6,width=6)
distance_percentile %>%
  inner_join(final_diagnostic) %>%
  dplyr::filter(species != "Deb", species != "Ano") %>%
  ggplot() +
...
t.test(distance_percentile %>%
         inner_join(final_diagnostic) %>%
         filter(species == "Ann") %>%
         pull(percent_score), mu=0.5)
t.test(distance_percentile %>%
         inner_join(final_diagnostic) %>%
         filter(species == "Arg") %>%
         pull(percent_score), mu=0.5)
