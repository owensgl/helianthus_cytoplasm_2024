...
percent_var_explained <- prop_var_explained * 100

# Print the results
for(i in 1:length(percent_var_explained)) {
  cat(sprintf("PC%d: %.2f%%\n", i, percent_var_explained[i]))
}


factoextra::fviz_eig(pca_result, addlabels = TRUE)
plot3 <- factoextra::fviz_contrib(pca_result, choice = "var", axes = 3, top = 10, 
...
plot1 <- analysis_data %>%
  ...
  geom_point() +
  scale_color_viridis_c(name="CP Clade 1\nproportion") +
...
pdf("figures/cp_pca.plots.v1.pdf",height=5,width=9)
plot1 + plot2 + plot3 +
...
write_tsv(results_df, "data/pca_cp_results.txt")
