...
cp_freq <- read_tsv("/media/drive_5_usb/speedy/working/cytoplasm/pop_gwas/argophyllus/cp.clade1.2.popfreq.txt",
                    col_names = c("pop","freq"))

cp_row <- cp_freq$freq

snp_freq <- read_tsv("/media/drive_5_usb/speedy/working/cytoplasm/pop_gwas/argophyllus/Argophyllus.tranche90.snp.gwas.90.bi.remappedHa412HO.popfreq.txt")
...
write_tsv(tmp, "/media/drive_5_usb/speedy/working/cytoplasm/pop_gwas/argophyllus/Argophyllus.tranche90.snp.gwas.90.bi.remappedHa412HO.cp.cor.txt")
...
cbind(tmp,window_name ) %>%
  ...
  write_csv(.,"/media/drive_5_usb/speedy/working/cytoplasm/pop_gwas/argophyllus/Argophyllus.tranche90.snp.gwas.90.bi.remappedHa412HO.wzainput.txt")
...
# 
# ###Example site
# snp_freq %>% filter(pos == 125332906) %>% 
#   dplyr::select(-pos) %>%
#   pivot_longer(-c(chr), names_to = "pop",values_to = "snp_freq") %>%
#   inner_join(cp_freq)  %>%
#   ggplot(.,aes(y=snp_freq,x=freq)) +
#   geom_point() + 
